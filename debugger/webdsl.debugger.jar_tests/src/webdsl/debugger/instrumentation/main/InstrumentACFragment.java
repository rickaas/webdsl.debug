...
public class InstrumentACFragment extends EventInstrumentationBase {
	...
	@Test
	public void instrumentYellowGrassAccessControlFragment() {
		this.initHI();
		...
		sourceAppDirectory = WebdslTestConstants.WEBDSL_SCRIPTS_TESTCASES_DIR + "/fragments/accesscontrol";
		...
		Assert.assertEquals(3, instrumentedFiles.size());
		for(String filename : instrumentedFiles) {
			compareWebDSLFileContents(getInstrumentedLocation() + "/" + filename, output + "/" + filename);
		}
