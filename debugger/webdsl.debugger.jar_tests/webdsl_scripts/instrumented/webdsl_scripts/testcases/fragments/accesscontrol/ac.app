...
access control rules
  ...
  {
  [ p | p : Project in ps where ! mayAccess(p) ].length
  ==
  0
  }
  predicate
  ...
  rule page edit ( p : Project ) {
    ...
    rule action save (  ) {
      principal in p.members
        ...
        principal in p.members && p.members.length > 1
  ...
  rule template projectMembershipRequests ( p : Project ) {
    principal in p.members && p.memberRequests.length > 0
  }
  ...
  rule ajaxtemplate roadmapRelease ( r : Tag ) {
    ...
    rule action postponeOpen ( release : Tag ) {
      principal in release.project.members && release.isRelease() && ! releaseDone(release) && nextRelease(release) != null
  ...
  rule template projectTools ( p : Project ) {
    ...
    rule action followProject ( p : Project ) {
      loggedIn && ! ( principal in p.followers ) && ! ( principal in p.members ) && ! ( principal in p.memberRequests )
    } rule action unfollowProject ( p : Project ) {
        loggedIn && principal in p.followers
      } rule action requestJoinProject ( p : Project ) {
          loggedIn && ! ( principal in p.members ) && ! ( principal in p.memberRequests )
        }
  }
